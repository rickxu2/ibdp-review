...
4. 每份 assignment 的全部逐题 attempts 必须共享同一个 `source.assignment_id`（`ASG-YYYYMMDD-NNN`）；同一天多份 assignment 分别编号。不得把 submission UUID、学生姓名或私有文件路径用作公开 assignment id。
5. JSON 改完格式必须合法（宁可 python -m json.tool 验一下）。
